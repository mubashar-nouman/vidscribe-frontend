...
const Hero = () => {
    return (
        <div className="flex flex-col items-center text-center w-full max-w-5xl my-24 mx-auto z-40 relative">
            <Container delay={0.0}>
                <div className="pl-2 pr-1 py-1 rounded-full border border-border/50 hover:border-border/70 backdrop-blur-lg cursor-pointer flex items-center gap-2.5 select-none w-max mx-auto">
            ...
            <BlurText
                word={"Turn Any Video Into Ready-to-Publish Content in Seconds"}
                className="text-3xl sm:text-5xl lg:text-6xl bg-gradient-to-br from-foreground to-foreground/60 bg-clip-text text-transparent py-2 md:py-0 lg:!leading-snug font-bold racking-[-0.0125em] mt-6 font-heading"
            />
            <Container delay={0.1}>
                <p className="text-sm sm:text-base lg:text-lg mt-4 text-accent-foreground/60 max-w-2xl mx-auto">
                Upload your video or paste a link into {process.env.NEXT_PUBLIC_APP_NAME} and let it do the magic. Instantly turn your content into blogs, social posts, newsletters and summaries to maximize reach and save time.
                </p>
            </Container>
            <Container delay={0.2}>
            ...
            <Container delay={0.3}>
                <div className="relative mx-auto max-w-7xl rounded-xl lg:rounded-[32px] border border-border/50 p-2 backdrop-blur-lg bg-card/50 md:p-4 mt-12">
                    <div className="absolute top-1/4 left-1/2 -z-10 gradient w-3/4 -translate-x-1/2 h-1/4 -translate-y-1/2 inset-0 blur-[10rem]"></div>

                    <div className="rounded-lg lg:rounded-[24px] border p-2 border-border bg-background">
                        <Image
